...
def generate_response(predefined_length=750, model_name='', lora_model_path='', audio_config='', audio_path_root='', target_path=''):
    ...
    for i in range(length):
        ...
        # For Long Audio chunks, model should truncate the overall audio into several chunks
        length = math.ceil(audios[0].shape[0] / processor.feature_extractor.n_samples)
        for i in range(length):
            audios.append(audios[0][i*processor.feature_extractor.n_samples:(i+1)*processor.feature_extractor.n_samples])
        audios = audios[1:]
        ...
        generate_ids = model.generate(**inputs, long_speech_processing=True)
