...
class Qwen2AudioEncoder(Qwen2AudioPreTrainedModel):
    ...
    def forward(
        ...
        # adding pooling operation
        #hidden_states = self.avg_pooler(hidden_states)
        
        hidden_states = hidden_states.permute(0, 2, 1)
        # merge hidden states
        # if predefined_length is not None:
        #     pooled_mask = attention_mask[:, 0, 0, ::2] == 0.0
        #     hidden_states = self.memory_merge(hidden_states, pooled_mask, audio_output_lengths, predefined_length)
        ...
        # Append new hidden_states
        return BaseModelOutput(
            last_hidden_state=hidden_states, hidden_states=tmp_hidden_states, attentions=all_attentions
        )
        '''
        return BaseModelOutput(
    ...
    def memory_merge(self, audio_features, feature_attention_mask, audio_output_lengths, predefined_length):
        bsz, length, feature_len = audio_features.size()
        final_features = None
        #audio_lengths = feature_attention_mask.sum(dim=-1)

        for i in range(bsz):
            if audio_output_lengths[i] < predefined_length:
                tmp_feature = audio_features[i, :predefined_length, :]
            else:
                dim = audio_features.size(-1)
                tmp_feature = audio_features[i][feature_attention_mask[i].unsqueeze(-1).expand(audio_features[i].size(0), dim)].contiguous().view(-1, dim)
                tmp_feature = self.pooled_summarize(tmp_feature, predefined_length)
            if final_features is None:
                final_features = tmp_feature.unsqueeze(0)
            else:
                final_features = torch.cat([final_features, tmp_feature.unsqueeze(0)], dim=0)
        return final_features
    def pooled_summarize(self, audio_features, predefined_length):
        length = audio_features.size(0)
        times = length / predefined_length
        merge_list = []
        j = 0
        for i in range(predefined_length):
            tmp_features = None
            summation_times = 0
            while j < (i+1)*times:
                if tmp_features is None:
                    tmp_features = audio_features[j]
                else:
                    tmp_features = tmp_features + audio_features[j]
                j += 1
                summation_times += 1
            merge_list.append((tmp_features / summation_times).unsqueeze(0))
        final_features = torch.cat(merge_list, dim=0)
        print(final_features.size())
        return final_features
    
    def summarize(self, audio_features, predefined_length):
        length = audio_features.size(0)
        ...
        merge_times = torch.ones(length).to(pre_feature.device)
        res_length = length - predefined_length

        for i in range(res_length):
            _, indices = torch.max(similarity, dim=0)
            audio_features = torch.cat([audio_features[:indices], (audio_features[indices] * (merge_times[indices] / (merge_times[indices] + merge_times[indices+1])) + audio_features[indices+1] * (merge_times[indices+1] / (merge_times[indices] + merge_times[indices+1]))).unsqueeze(0), audio_features[indices+2:]], dim=0)

            if audio_features.size(0) <= predefined_length:
                return audio_features
            similarity = torch.cat([similarity[:indices], similarity[indices+1:]], dim=0)
            merge_times[indices] = merge_times[indices] + merge_times[indices+1]
            merge_times = torch.cat([merge_times[:indices], merge_times[indices+1:]], dim=0)
            if indices - 1 >= 0:
                similarity[indices-1] = torch.dot(audio_features[indices-1], audio_features[indices]) / (torch.norm(audio_features[indices-1]) * torch.norm(audio_features[indices]))
            if indices + 1 < audio_features.size(0):
                similarity[indices] = torch.dot(audio_features[indices], audio_features[indices+1]) / (torch.norm(audio_features[indices]) * torch.norm(audio_features[indices+1]))
        return audio_features
    # Ignore copy
    ...
    # Ignore copy
    # def _get_feat_extract_output_lengths_predefine(self, input_lengths: torch.LongTensor, predefined_length):
    #     """
    #     Computes the output length of the convolutional layers and the output length of the audio encoder
    #     """
    #     input_lengths = (input_lengths - 1) // 2 + 1
    ...
    #     # adding pooling layers
    #     #output_lengths = (output_lengths - 2) // 2 + 1
    #     if predefined_length is not None and (output_lengths > predefined_length).any():
    #         #output_lengths = torch.ones(input_lengths.size(0), predefined_length).to(input_lengths.device)
    #         output_lengths[output_lengths > predefined_length] = predefined_length
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if input_features is not None and input_ids.shape[1] != 1:
                ...
                '''
                # new_method
                predefined_length = 75
                audio_feat_lengths, audio_output_lengths = self.audio_tower._get_feat_extract_output_lengths_predefine(
                    feature_attention_mask.sum(-1), predefined_length
                )
